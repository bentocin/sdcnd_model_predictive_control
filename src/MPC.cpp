...
// Thus, we should establish when one variable starts and another ends.
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // The weights for the cost terms
    const int w_cte = 2500;
    const int w_epsi = 2500;
    const int w_v = 1;
    const int w_delta = 10;
    const int w_a = 10;
    const int w_ddelta = 150;
    const int w_da = 10;

    // The cost related the reference state and anything that my be beneficial
    for (int i = 0; i < N; i++) {
    ...
    // Minimize the use of actuators to reduce erratic control inputs
    for (int i = 0; i < N - 1; i++) {
      fg[0] += w_delta * CppAD::pow(vars[delta_start + i], 2);
      fg[0] += w_a * CppAD::pow(vars[a_start + i], 2);
    }

    // Minimize the value gap between sequential actuations to smooth the output
    for (int i = 0; i < N - 2; i++) {
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Return the actuator values needed and the (x, y) values to show in the simulator
  vector<double> res;
